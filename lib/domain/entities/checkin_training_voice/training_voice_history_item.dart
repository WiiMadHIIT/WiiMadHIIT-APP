/// 语音训练历史记录项实体
class TrainingVoiceHistoryItem {
  ...
  final String? note; // 用于标识当前训练结果（前端生成，不从后端获取）

  TrainingVoiceHistoryItem({
  ...
  /// 复制并更新字段
  TrainingVoiceHistoryItem copyWith({
    String? id,
    int? rank,
    int? counts,
    double? countsPerMin,
  ...
  /// 从Map创建
  factory TrainingVoiceHistoryItem.fromMap(Map<String, dynamic> map) {
    return TrainingVoiceHistoryItem(
      id: map['id'] as String?,
  ...
  /// 创建当前训练结果的历史项
  factory TrainingVoiceHistoryItem.createCurrent({
    required String id,
    required int counts,
  ...
  /// 创建加载中的历史项
  factory TrainingVoiceHistoryItem.createLoading({
    required String id,
    required int counts,
    required double countsPerMin,
    required int timestamp,
  ...
  @override
  String toString() {
    return 'TrainingVoiceHistoryItem(id: $id, rank: $rank, counts: $counts, countsPerMin: $countsPerMin, timestamp: $timestamp, note: $note)';
  }

  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
    return other is TrainingVoiceHistoryItem &&
        other.id == id &&
        other.rank == rank &&
        other.counts == counts &&
